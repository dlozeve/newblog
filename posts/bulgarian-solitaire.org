...
date: 2026-02-08
tags: paper, maths, dynamical systems
toc: false
---
...
sequence of numbers that sum to $n$. For instance, $(4,3,2,1)$ and
...
3, and 4. It turns out that this pattern can be observed consistently:
...
Sometimes we can see that a box (in grey here) is above an empty
...
So now we just need to prove that all configurations will end up at
...
detailed proof can be found in section 2 of [[https://arxiv.org/abs/1503.00885][the paper]].
...
$0 < k \leq n$, the partitions we are looking for consist of $k$
followed by all partitions of $n-k$.
...
Now for display, we generate some [[https://graphviz.org/][Graphviz]] code. The colors are
...
independently with probability $0<p<1$. The dynamical system becomes a
Markov chain, and it would be interesting to study (even
experimentally) its convergence properties.
...
The ~Partitions~ function generates all partitions of a given
integer. Conveniently, this was already available in [[https://mlochbaum.github.io/bqncrate/?q=integer%20partitions][BQNcrate]]!
#+begin_src txt
Partitions←{∾⊢´∾⟜(<-⟜↕∘≠{𝕨∾¨∾(-𝕨⌊≠𝕩)↑𝕩}¨⊢)⍟𝕩⋈⋈⋈↕0}
#+end_src

We can now compute the graph, by first computing all partitions of the
